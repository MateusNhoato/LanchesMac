...
namespace LanchesMac.Areas.Admin.Controllers
{
    ...
    public class AdminImagensController : Controller
    {
        ...
        private IWebHostEnvironment _hostingEnvirontment;

        public AdminImagensController(IOptions<ConfigurationImagens> myConfig, 
            IWebHostEnvironment hostingEnvirontment)
        {
        ...
        public async Task<IActionResult> UploadFiles(List<IFormFile> files)
        {
            ...
            var filePath = Path.Combine(_hostingEnvirontment.WebRootPath,
        ...
        public IActionResult GetImagens()
        {
            ...
            var userImagesPath = Path.Combine(_hostingEnvirontment.WebRootPath,
                _myConfig.NomePastaImagensProdutos);
        ...
        {
            string _imagemDeletada = Path.Combine(_hostingEnvirontment.WebRootPath,
                _myConfig.NomePastaImagensProdutos + "\\", fname);

            if(System.IO.File.Exists(_imagemDeletada)) 
            {
                System.IO.File.Delete(_imagemDeletada);
                ViewData["Deletado"] = $"Arquivos(s) {_imagemDeletada} deletado com sucesso";
            }
            return View("index");
        }
    }
